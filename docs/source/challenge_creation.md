...
One can create a challenge in EvalAI using :

1. zip configuration
2. web interface

## Creating a challenge using zip configuration
...
The challenge configuration file on EvalAI consists of the following fields:

* **title**:  The Title of the challenge

* **short_description**: AShort description of the challenge (preferably 140 characters max)

* **description**:A Long description of the challenge (use a relative path for the html file, e.g. `challenge_details/description.html`)
...
* **image**: Logo of the challenge (use a relative path for the logo in the zip configuration, e.g. `images/logo/challenge_logo.jpg`). **Note**: The image must be in jpg, jpeg or png format only.
...
* **start_date**: Specifies the starting date and time of the challenge (Format: YYYY-MM-DD HH:MM:SS, e.g. 2017-07-07 10:10:10)

* **end_date**: Specifies the end date and time of the challenge (Format: YYYY-MM-DD HH:MM:SS, e.g. 2017-07-07 10:10:10)
...
* **leaderboard**:
  ...
  A leaderboard in a challenge on EvalAI consists of the following subfields:

  * **id**: Unique integer field for each leaderboard entry

  * **schema**: Schema field contains the information about the rows of the leaderboard. It mainly contains two keys in the leaderboard:

  1. `labels`: Labels are the header rows in the leaderboard according to which the challenge of ranking is done.
...
* **challenge_phases**:
  ...
  There can be multiple challenge phases in a challenge. A challenge phase in a challenge contains the following subfields:
    ...
    * **start_date**: Specifies the starting date and time of the challenge phase (Format: YYYY-MM-DD HH:MM:SS, e.g. 2017-07-07 10:10:10)

    * **end_date**: Specifies the ending date and time of the challenge phase (Format: YYYY-MM-DD HH:MM:SS, e.g. 2017-07-07 10:10:10)
    ...
    * **codename**: Challenge phase codename. Note that the codename of a challenge phase is used to map the results returned by the evaluation script to a particular challenge phase. The codename specified here should match the codename specified in the evaluation script to perfect mapping.

    * **max_submissions_per_day**: Positive integer  tells the maximum number of submissions per day to a challenge phase.
...
* **dataset_splits**:
  ...
  * **codename**: Codename of dataset split. Note that the codename of a dataset split is used to map the results returned by the evaluation script to a particular dataset split in EvalAI's database. Please make sure that no other dataset splits have the same codename. Also make sure that the dataset split's codename matches with what is in the evaluation script provided by the challenge host.


* **challenge_phase_splits**:

  A challenge phase split is a relation between a challenge phase and dataset splits of a challenge (many to many relation). This is used to set the privacy of submissions (public/private) to different dataset splits for different challenge phases.
  ...
  * **visibility**: Enters only the  positive integers given below.
...
The final step is to create a challenge host team for the challenge on EvalAI. After that, just upload the zip folder created by the above steps and the challenge will be created.

If you have any issues in creating a challenge on EvalAI, please feel free to create an issue on our Github Issues Page.
